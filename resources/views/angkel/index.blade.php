...
<?php
$acl = \App\Models\Role::where('id', \Auth::user()->role_id)->where('route_access_list', 'LIKE', '%'.'angkel.index'.'%')->count();
            ...
            <table style="width:100%" id='tb_ini' class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline">
              ...
              <?php $x=1; ?>
                  @foreach($data as $row)
                      <tr>
                          <th>{{ $x }}</th>
                          <th><a href="{{ url('individu') }}/{{ $row->penduduk->nik }}">{{ $row->penduduk->nik }}</a></th>
                          <th><?php echo ucwords($row->penduduk->nama_lengkap) ?></th>
                          <th><?php echo ucwords($row->penduduk->status_hubungan_keluarga) ?></th>
                          <th>{{ $row->penduduk->jk or ''}}</th>
                          <th>{{ $row->penduduk->tgl_lahir or '' }}</th>

                          <th align="center">
                            <?php if(isset($row->penduduk)) { ?>
                            <a class="btn btn-primary" href="{{ url('penduduk/edit') }}/{{ $row->penduduk->id }}">Ubah</a>
                            <a onclick="return hapus()" href="{{url('penduduk/delete')}}/{{$row->penduduk->id}}" type="submit" class="btn btn-danger">Hapus</a>
                            <?php } ?>
                          </th>
