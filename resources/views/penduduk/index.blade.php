...
<?php
$acl = \App\Models\Role::where('id', \Auth::user()->role_id)->where('route_access_list', 'LIKE', '%'.'penduduk.index'.'%')->count();
        ...
        <div class="card-body">
          <div class="table-responsive">
            <table style="width:100%" id='tb_ini'  class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline">
              <thead class=" text-primary textcenter">
                <th> No </th>
                <th> NIK </th>
                <th> Nama </th>
                <th> JK </th>
              ...
              <?php $x=1; ?>
                  @foreach($data as $row)
                      <tr>
                          <th>{{ $x }}</th>
                          <th><a href="{{ url('individu') }}/{{ $row->nik }}">{{ $row->nik }}</a></th>
